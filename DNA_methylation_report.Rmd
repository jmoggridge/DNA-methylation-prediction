...
```{r}
# what about PCA by dinucleotide composition
read_csv()
```
...
svm_cv <- read_csv("./results/svm_cv_results.csv") %>% 
  transmute(
    ...
    gamma = round(param_svm__gamma, 3),
    params,
    mean = mean_test_score,
    std_err = std_test_score
  )
...
```{r}
read_rds("./figures/fig_islands.rds")
```

```{r}
read_rds("./figures/fig_chromosomes.rds")
```



```{r}
read_rds("./figures/fig_refgene_groups.rds")
```

```{r}
read_rds("./figures/fig_dinucleotide.rds")
```
